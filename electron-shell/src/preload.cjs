...
contextBridge.exposeInMainWorld("qrSuite", {
  ...
  selectRecordingRegion: (payload) => ipcRenderer.invoke("recording:selectRegion", payload),
  ...
  filePickerList: (token, payload) => ipcRenderer.invoke(`filePicker:list:${token}`, payload),
  filePickerChoose: (token, payload) => ipcRenderer.invoke(`filePicker:choose:${token}`, payload),
  filePickerCancel: (token) => ipcRenderer.invoke(`filePicker:cancel:${token}`),
  regionSelectorFinish: (token, payload) => ipcRenderer.invoke(`regionSelector:finish:${token}`, payload),
  regionSelectorCancel: (token) => ipcRenderer.invoke(`regionSelector:cancel:${token}`),
