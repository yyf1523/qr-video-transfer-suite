...
class DecoderGui:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.screen_60fps_var = tk.BooleanVar(value=False)
        self.screen_grid_var = tk.BooleanVar(value=True)
        self.screen_fast_fec_var = tk.BooleanVar(value=True)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(options, text="4QR/30FPS/FEC", variable=self.screen_fast_fec_var, command=self.apply_fast_fec_defaults).grid(row=0, column=3, sticky="w")
        ttk.Label(options, text="每 N 帧解一次").grid(row=0, column=4, sticky="e")
        ttk.Entry(options, textvariable=self.every_n_var, width=10).grid(row=0, column=5, sticky="ew", padx=(6, 12))
        ttk.Label(options, text="投票窗口").grid(row=0, column=6, sticky="e")
        ttk.Entry(options, textvariable=self.vote_window_var, width=10).grid(row=0, column=7, sticky="ew", padx=(6, 12))
    ...
    def apply_fast_fec_defaults(self) -> None:
        if self.screen_fast_fec_var.get():
            self.screen_mode_var.set(True)
            self.screen_60fps_var.set(False)
            self.screen_grid_var.set(True)
            self.record_fps_var.set("30")
            self.every_n_var.set("1")
            self.vote_window_var.set("1")
    ...
    def build_command(self) -> list[str]:
        ...
        self.state_var.set(str((Path.cwd() / "decode_state" / f"session-{stamp}").resolve()))
        ...
        if self.screen_fast_fec_var.get():
            command.append("--screen-fast-fec-4qr")
